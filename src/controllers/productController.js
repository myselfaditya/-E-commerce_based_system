...
const { isValidRequestBody, isItEmpty, validSizes, isValidPrice, isValidNumber, isValidDigit, isValidfild, urlreg } = require("../validator/validation")
...
const createProduct = async function (req, res) {
    try {
        ...
        if (!isItEmpty(title)) return res.status(400).send({ status: false, message: "Title is required" })
        let titleIn = await productModel.findOne({ title: title })
        if (titleIn) return res.status(400).send({ status: false, message: "Title is already used it must be Unique" })

        if (!isItEmpty(description)) return res.status(400).send({ status: false, message: "Description is required" })
        ...
        if (!isItEmpty(currencyFormat)) return res.status(400).send({ status: false, message: "currencyformat is required" });
        if (currencyFormat) {
        ...
        if (style) { if (!isItEmpty(style)) return res.status(400).send({ status: false, message: "Provie a valid style" }) }
...
const getProductByQuery = async function (req, res) {
    try {
        ...
        let { size, name, priceGreaterThan, priceLessThan, priceSort, page = 1, limit = 10 } = queryData
        ...
        if (size || priceSort || priceLessThan || priceGreaterThan || name) {
            ...
            let findFilter = await productModel.find(objectFilter).sort({ price: priceSort }).limit(limit * 1).skip((page - 1) * limit)
