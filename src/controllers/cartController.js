...
const { isValidRequestBody, isValidNumber, isItEmpty, isValidNumber } = require("../validator/validation")
...
const updateCart = async function (req, res) {
    try {
        ...
        if (!isItEmpty(removeProduct)) return res.status(400).send({ status: false, message: "removeProduct value 0 or 1 is required" })
        if (!isItEmpty(productId)) return res.status(400).send({ status: false, message: "productId is required" })
        if (!isValidNumber(removeProduct)) return res.status(400).send({ status: false, message: "productId is required" })
